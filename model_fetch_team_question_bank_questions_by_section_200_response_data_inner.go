/*
HDRUK Gateway API

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 2.0.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package gatewayapisdk

import (
	"encoding/json"
	"time"
)

// checks if the FetchTeamQuestionBankQuestionsBySection200ResponseDataInner type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &FetchTeamQuestionBankQuestionsBySection200ResponseDataInner{}

// FetchTeamQuestionBankQuestionsBySection200ResponseDataInner struct for FetchTeamQuestionBankQuestionsBySection200ResponseDataInner
type FetchTeamQuestionBankQuestionsBySection200ResponseDataInner struct {
	Id *int32 `json:"id,omitempty"`
	CreatedAt *time.Time `json:"created_at,omitempty"`
	UpdatedAt *time.Time `json:"updated_at,omitempty"`
	DeletedAt *time.Time `json:"deleted_at,omitempty"`
	SectionId *int32 `json:"section_id,omitempty"`
	UserId *int32 `json:"user_id,omitempty"`
	Locked *bool `json:"locked,omitempty"`
	Archived *bool `json:"archived,omitempty"`
	ArchivedDate *time.Time `json:"archived_date,omitempty"`
	ForceRequired *bool `json:"force_required,omitempty"`
	AllowGuidanceOverride *bool `json:"allow_guidance_override,omitempty"`
	IsChild *bool `json:"is_child,omitempty"`
	QuestionType *string `json:"question_type,omitempty"`
	LatestVersion map[string]interface{} `json:"latest_version,omitempty"`
	Versions map[string]interface{} `json:"versions,omitempty"`
}

// NewFetchTeamQuestionBankQuestionsBySection200ResponseDataInner instantiates a new FetchTeamQuestionBankQuestionsBySection200ResponseDataInner object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewFetchTeamQuestionBankQuestionsBySection200ResponseDataInner() *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner {
	this := FetchTeamQuestionBankQuestionsBySection200ResponseDataInner{}
	return &this
}

// NewFetchTeamQuestionBankQuestionsBySection200ResponseDataInnerWithDefaults instantiates a new FetchTeamQuestionBankQuestionsBySection200ResponseDataInner object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewFetchTeamQuestionBankQuestionsBySection200ResponseDataInnerWithDefaults() *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner {
	this := FetchTeamQuestionBankQuestionsBySection200ResponseDataInner{}
	return &this
}

// GetId returns the Id field value if set, zero value otherwise.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) GetId() int32 {
	if o == nil || IsNil(o.Id) {
		var ret int32
		return ret
	}
	return *o.Id
}

// GetIdOk returns a tuple with the Id field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) GetIdOk() (*int32, bool) {
	if o == nil || IsNil(o.Id) {
		return nil, false
	}
	return o.Id, true
}

// HasId returns a boolean if a field has been set.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) HasId() bool {
	if o != nil && !IsNil(o.Id) {
		return true
	}

	return false
}

// SetId gets a reference to the given int32 and assigns it to the Id field.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) SetId(v int32) {
	o.Id = &v
}

// GetCreatedAt returns the CreatedAt field value if set, zero value otherwise.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) GetCreatedAt() time.Time {
	if o == nil || IsNil(o.CreatedAt) {
		var ret time.Time
		return ret
	}
	return *o.CreatedAt
}

// GetCreatedAtOk returns a tuple with the CreatedAt field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) GetCreatedAtOk() (*time.Time, bool) {
	if o == nil || IsNil(o.CreatedAt) {
		return nil, false
	}
	return o.CreatedAt, true
}

// HasCreatedAt returns a boolean if a field has been set.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) HasCreatedAt() bool {
	if o != nil && !IsNil(o.CreatedAt) {
		return true
	}

	return false
}

// SetCreatedAt gets a reference to the given time.Time and assigns it to the CreatedAt field.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) SetCreatedAt(v time.Time) {
	o.CreatedAt = &v
}

// GetUpdatedAt returns the UpdatedAt field value if set, zero value otherwise.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) GetUpdatedAt() time.Time {
	if o == nil || IsNil(o.UpdatedAt) {
		var ret time.Time
		return ret
	}
	return *o.UpdatedAt
}

// GetUpdatedAtOk returns a tuple with the UpdatedAt field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) GetUpdatedAtOk() (*time.Time, bool) {
	if o == nil || IsNil(o.UpdatedAt) {
		return nil, false
	}
	return o.UpdatedAt, true
}

// HasUpdatedAt returns a boolean if a field has been set.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) HasUpdatedAt() bool {
	if o != nil && !IsNil(o.UpdatedAt) {
		return true
	}

	return false
}

// SetUpdatedAt gets a reference to the given time.Time and assigns it to the UpdatedAt field.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) SetUpdatedAt(v time.Time) {
	o.UpdatedAt = &v
}

// GetDeletedAt returns the DeletedAt field value if set, zero value otherwise.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) GetDeletedAt() time.Time {
	if o == nil || IsNil(o.DeletedAt) {
		var ret time.Time
		return ret
	}
	return *o.DeletedAt
}

// GetDeletedAtOk returns a tuple with the DeletedAt field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) GetDeletedAtOk() (*time.Time, bool) {
	if o == nil || IsNil(o.DeletedAt) {
		return nil, false
	}
	return o.DeletedAt, true
}

// HasDeletedAt returns a boolean if a field has been set.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) HasDeletedAt() bool {
	if o != nil && !IsNil(o.DeletedAt) {
		return true
	}

	return false
}

// SetDeletedAt gets a reference to the given time.Time and assigns it to the DeletedAt field.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) SetDeletedAt(v time.Time) {
	o.DeletedAt = &v
}

// GetSectionId returns the SectionId field value if set, zero value otherwise.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) GetSectionId() int32 {
	if o == nil || IsNil(o.SectionId) {
		var ret int32
		return ret
	}
	return *o.SectionId
}

// GetSectionIdOk returns a tuple with the SectionId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) GetSectionIdOk() (*int32, bool) {
	if o == nil || IsNil(o.SectionId) {
		return nil, false
	}
	return o.SectionId, true
}

// HasSectionId returns a boolean if a field has been set.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) HasSectionId() bool {
	if o != nil && !IsNil(o.SectionId) {
		return true
	}

	return false
}

// SetSectionId gets a reference to the given int32 and assigns it to the SectionId field.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) SetSectionId(v int32) {
	o.SectionId = &v
}

// GetUserId returns the UserId field value if set, zero value otherwise.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) GetUserId() int32 {
	if o == nil || IsNil(o.UserId) {
		var ret int32
		return ret
	}
	return *o.UserId
}

// GetUserIdOk returns a tuple with the UserId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) GetUserIdOk() (*int32, bool) {
	if o == nil || IsNil(o.UserId) {
		return nil, false
	}
	return o.UserId, true
}

// HasUserId returns a boolean if a field has been set.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) HasUserId() bool {
	if o != nil && !IsNil(o.UserId) {
		return true
	}

	return false
}

// SetUserId gets a reference to the given int32 and assigns it to the UserId field.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) SetUserId(v int32) {
	o.UserId = &v
}

// GetLocked returns the Locked field value if set, zero value otherwise.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) GetLocked() bool {
	if o == nil || IsNil(o.Locked) {
		var ret bool
		return ret
	}
	return *o.Locked
}

// GetLockedOk returns a tuple with the Locked field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) GetLockedOk() (*bool, bool) {
	if o == nil || IsNil(o.Locked) {
		return nil, false
	}
	return o.Locked, true
}

// HasLocked returns a boolean if a field has been set.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) HasLocked() bool {
	if o != nil && !IsNil(o.Locked) {
		return true
	}

	return false
}

// SetLocked gets a reference to the given bool and assigns it to the Locked field.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) SetLocked(v bool) {
	o.Locked = &v
}

// GetArchived returns the Archived field value if set, zero value otherwise.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) GetArchived() bool {
	if o == nil || IsNil(o.Archived) {
		var ret bool
		return ret
	}
	return *o.Archived
}

// GetArchivedOk returns a tuple with the Archived field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) GetArchivedOk() (*bool, bool) {
	if o == nil || IsNil(o.Archived) {
		return nil, false
	}
	return o.Archived, true
}

// HasArchived returns a boolean if a field has been set.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) HasArchived() bool {
	if o != nil && !IsNil(o.Archived) {
		return true
	}

	return false
}

// SetArchived gets a reference to the given bool and assigns it to the Archived field.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) SetArchived(v bool) {
	o.Archived = &v
}

// GetArchivedDate returns the ArchivedDate field value if set, zero value otherwise.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) GetArchivedDate() time.Time {
	if o == nil || IsNil(o.ArchivedDate) {
		var ret time.Time
		return ret
	}
	return *o.ArchivedDate
}

// GetArchivedDateOk returns a tuple with the ArchivedDate field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) GetArchivedDateOk() (*time.Time, bool) {
	if o == nil || IsNil(o.ArchivedDate) {
		return nil, false
	}
	return o.ArchivedDate, true
}

// HasArchivedDate returns a boolean if a field has been set.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) HasArchivedDate() bool {
	if o != nil && !IsNil(o.ArchivedDate) {
		return true
	}

	return false
}

// SetArchivedDate gets a reference to the given time.Time and assigns it to the ArchivedDate field.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) SetArchivedDate(v time.Time) {
	o.ArchivedDate = &v
}

// GetForceRequired returns the ForceRequired field value if set, zero value otherwise.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) GetForceRequired() bool {
	if o == nil || IsNil(o.ForceRequired) {
		var ret bool
		return ret
	}
	return *o.ForceRequired
}

// GetForceRequiredOk returns a tuple with the ForceRequired field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) GetForceRequiredOk() (*bool, bool) {
	if o == nil || IsNil(o.ForceRequired) {
		return nil, false
	}
	return o.ForceRequired, true
}

// HasForceRequired returns a boolean if a field has been set.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) HasForceRequired() bool {
	if o != nil && !IsNil(o.ForceRequired) {
		return true
	}

	return false
}

// SetForceRequired gets a reference to the given bool and assigns it to the ForceRequired field.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) SetForceRequired(v bool) {
	o.ForceRequired = &v
}

// GetAllowGuidanceOverride returns the AllowGuidanceOverride field value if set, zero value otherwise.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) GetAllowGuidanceOverride() bool {
	if o == nil || IsNil(o.AllowGuidanceOverride) {
		var ret bool
		return ret
	}
	return *o.AllowGuidanceOverride
}

// GetAllowGuidanceOverrideOk returns a tuple with the AllowGuidanceOverride field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) GetAllowGuidanceOverrideOk() (*bool, bool) {
	if o == nil || IsNil(o.AllowGuidanceOverride) {
		return nil, false
	}
	return o.AllowGuidanceOverride, true
}

// HasAllowGuidanceOverride returns a boolean if a field has been set.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) HasAllowGuidanceOverride() bool {
	if o != nil && !IsNil(o.AllowGuidanceOverride) {
		return true
	}

	return false
}

// SetAllowGuidanceOverride gets a reference to the given bool and assigns it to the AllowGuidanceOverride field.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) SetAllowGuidanceOverride(v bool) {
	o.AllowGuidanceOverride = &v
}

// GetIsChild returns the IsChild field value if set, zero value otherwise.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) GetIsChild() bool {
	if o == nil || IsNil(o.IsChild) {
		var ret bool
		return ret
	}
	return *o.IsChild
}

// GetIsChildOk returns a tuple with the IsChild field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) GetIsChildOk() (*bool, bool) {
	if o == nil || IsNil(o.IsChild) {
		return nil, false
	}
	return o.IsChild, true
}

// HasIsChild returns a boolean if a field has been set.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) HasIsChild() bool {
	if o != nil && !IsNil(o.IsChild) {
		return true
	}

	return false
}

// SetIsChild gets a reference to the given bool and assigns it to the IsChild field.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) SetIsChild(v bool) {
	o.IsChild = &v
}

// GetQuestionType returns the QuestionType field value if set, zero value otherwise.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) GetQuestionType() string {
	if o == nil || IsNil(o.QuestionType) {
		var ret string
		return ret
	}
	return *o.QuestionType
}

// GetQuestionTypeOk returns a tuple with the QuestionType field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) GetQuestionTypeOk() (*string, bool) {
	if o == nil || IsNil(o.QuestionType) {
		return nil, false
	}
	return o.QuestionType, true
}

// HasQuestionType returns a boolean if a field has been set.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) HasQuestionType() bool {
	if o != nil && !IsNil(o.QuestionType) {
		return true
	}

	return false
}

// SetQuestionType gets a reference to the given string and assigns it to the QuestionType field.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) SetQuestionType(v string) {
	o.QuestionType = &v
}

// GetLatestVersion returns the LatestVersion field value if set, zero value otherwise.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) GetLatestVersion() map[string]interface{} {
	if o == nil || IsNil(o.LatestVersion) {
		var ret map[string]interface{}
		return ret
	}
	return o.LatestVersion
}

// GetLatestVersionOk returns a tuple with the LatestVersion field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) GetLatestVersionOk() (map[string]interface{}, bool) {
	if o == nil || IsNil(o.LatestVersion) {
		return map[string]interface{}{}, false
	}
	return o.LatestVersion, true
}

// HasLatestVersion returns a boolean if a field has been set.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) HasLatestVersion() bool {
	if o != nil && !IsNil(o.LatestVersion) {
		return true
	}

	return false
}

// SetLatestVersion gets a reference to the given map[string]interface{} and assigns it to the LatestVersion field.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) SetLatestVersion(v map[string]interface{}) {
	o.LatestVersion = v
}

// GetVersions returns the Versions field value if set, zero value otherwise.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) GetVersions() map[string]interface{} {
	if o == nil || IsNil(o.Versions) {
		var ret map[string]interface{}
		return ret
	}
	return o.Versions
}

// GetVersionsOk returns a tuple with the Versions field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) GetVersionsOk() (map[string]interface{}, bool) {
	if o == nil || IsNil(o.Versions) {
		return map[string]interface{}{}, false
	}
	return o.Versions, true
}

// HasVersions returns a boolean if a field has been set.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) HasVersions() bool {
	if o != nil && !IsNil(o.Versions) {
		return true
	}

	return false
}

// SetVersions gets a reference to the given map[string]interface{} and assigns it to the Versions field.
func (o *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) SetVersions(v map[string]interface{}) {
	o.Versions = v
}

func (o FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Id) {
		toSerialize["id"] = o.Id
	}
	if !IsNil(o.CreatedAt) {
		toSerialize["created_at"] = o.CreatedAt
	}
	if !IsNil(o.UpdatedAt) {
		toSerialize["updated_at"] = o.UpdatedAt
	}
	if !IsNil(o.DeletedAt) {
		toSerialize["deleted_at"] = o.DeletedAt
	}
	if !IsNil(o.SectionId) {
		toSerialize["section_id"] = o.SectionId
	}
	if !IsNil(o.UserId) {
		toSerialize["user_id"] = o.UserId
	}
	if !IsNil(o.Locked) {
		toSerialize["locked"] = o.Locked
	}
	if !IsNil(o.Archived) {
		toSerialize["archived"] = o.Archived
	}
	if !IsNil(o.ArchivedDate) {
		toSerialize["archived_date"] = o.ArchivedDate
	}
	if !IsNil(o.ForceRequired) {
		toSerialize["force_required"] = o.ForceRequired
	}
	if !IsNil(o.AllowGuidanceOverride) {
		toSerialize["allow_guidance_override"] = o.AllowGuidanceOverride
	}
	if !IsNil(o.IsChild) {
		toSerialize["is_child"] = o.IsChild
	}
	if !IsNil(o.QuestionType) {
		toSerialize["question_type"] = o.QuestionType
	}
	if !IsNil(o.LatestVersion) {
		toSerialize["latest_version"] = o.LatestVersion
	}
	if !IsNil(o.Versions) {
		toSerialize["versions"] = o.Versions
	}
	return toSerialize, nil
}

type NullableFetchTeamQuestionBankQuestionsBySection200ResponseDataInner struct {
	value *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner
	isSet bool
}

func (v NullableFetchTeamQuestionBankQuestionsBySection200ResponseDataInner) Get() *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner {
	return v.value
}

func (v *NullableFetchTeamQuestionBankQuestionsBySection200ResponseDataInner) Set(val *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) {
	v.value = val
	v.isSet = true
}

func (v NullableFetchTeamQuestionBankQuestionsBySection200ResponseDataInner) IsSet() bool {
	return v.isSet
}

func (v *NullableFetchTeamQuestionBankQuestionsBySection200ResponseDataInner) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableFetchTeamQuestionBankQuestionsBySection200ResponseDataInner(val *FetchTeamQuestionBankQuestionsBySection200ResponseDataInner) *NullableFetchTeamQuestionBankQuestionsBySection200ResponseDataInner {
	return &NullableFetchTeamQuestionBankQuestionsBySection200ResponseDataInner{value: val, isSet: true}
}

func (v NullableFetchTeamQuestionBankQuestionsBySection200ResponseDataInner) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableFetchTeamQuestionBankQuestionsBySection200ResponseDataInner) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


