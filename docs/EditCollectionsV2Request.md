# EditCollectionsV2Request

## Properties

Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**Name** | Pointer to **string** |  | [optional] 
**Description** | Pointer to **string** |  | [optional] 
**ImageLink** | Pointer to **string** |  | [optional] 
**Enabled** | Pointer to **bool** |  | [optional] 
**Keywords** | Pointer to **[]string** |  | [optional] 
**Datasets** | Pointer to [**[]CreateCollectionsIntegrationsRequestDatasetsInner**](CreateCollectionsIntegrationsRequestDatasetsInner.md) |  | [optional] 
**Dur** | Pointer to [**[]CreateCollectionsIntegrationsRequestDatasetsInner**](CreateCollectionsIntegrationsRequestDatasetsInner.md) |  | [optional] 
**Publications** | Pointer to [**[]CreateCollectionsIntegrationsRequestDatasetsInner**](CreateCollectionsIntegrationsRequestDatasetsInner.md) |  | [optional] 
**Collaborators** | Pointer to **[]int32** |  | [optional] 
**Public** | Pointer to **bool** |  | [optional] 
**Status** | Pointer to **string** |  | [optional] 

## Methods

### NewEditCollectionsV2Request

`func NewEditCollectionsV2Request() *EditCollectionsV2Request`

NewEditCollectionsV2Request instantiates a new EditCollectionsV2Request object
This constructor will assign default values to properties that have it defined,
and makes sure properties required by API are set, but the set of arguments
will change when the set of required properties is changed

### NewEditCollectionsV2RequestWithDefaults

`func NewEditCollectionsV2RequestWithDefaults() *EditCollectionsV2Request`

NewEditCollectionsV2RequestWithDefaults instantiates a new EditCollectionsV2Request object
This constructor will only assign default values to properties that have it defined,
but it doesn't guarantee that properties required by API are set

### GetName

`func (o *EditCollectionsV2Request) GetName() string`

GetName returns the Name field if non-nil, zero value otherwise.

### GetNameOk

`func (o *EditCollectionsV2Request) GetNameOk() (*string, bool)`

GetNameOk returns a tuple with the Name field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetName

`func (o *EditCollectionsV2Request) SetName(v string)`

SetName sets Name field to given value.

### HasName

`func (o *EditCollectionsV2Request) HasName() bool`

HasName returns a boolean if a field has been set.

### GetDescription

`func (o *EditCollectionsV2Request) GetDescription() string`

GetDescription returns the Description field if non-nil, zero value otherwise.

### GetDescriptionOk

`func (o *EditCollectionsV2Request) GetDescriptionOk() (*string, bool)`

GetDescriptionOk returns a tuple with the Description field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetDescription

`func (o *EditCollectionsV2Request) SetDescription(v string)`

SetDescription sets Description field to given value.

### HasDescription

`func (o *EditCollectionsV2Request) HasDescription() bool`

HasDescription returns a boolean if a field has been set.

### GetImageLink

`func (o *EditCollectionsV2Request) GetImageLink() string`

GetImageLink returns the ImageLink field if non-nil, zero value otherwise.

### GetImageLinkOk

`func (o *EditCollectionsV2Request) GetImageLinkOk() (*string, bool)`

GetImageLinkOk returns a tuple with the ImageLink field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetImageLink

`func (o *EditCollectionsV2Request) SetImageLink(v string)`

SetImageLink sets ImageLink field to given value.

### HasImageLink

`func (o *EditCollectionsV2Request) HasImageLink() bool`

HasImageLink returns a boolean if a field has been set.

### GetEnabled

`func (o *EditCollectionsV2Request) GetEnabled() bool`

GetEnabled returns the Enabled field if non-nil, zero value otherwise.

### GetEnabledOk

`func (o *EditCollectionsV2Request) GetEnabledOk() (*bool, bool)`

GetEnabledOk returns a tuple with the Enabled field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetEnabled

`func (o *EditCollectionsV2Request) SetEnabled(v bool)`

SetEnabled sets Enabled field to given value.

### HasEnabled

`func (o *EditCollectionsV2Request) HasEnabled() bool`

HasEnabled returns a boolean if a field has been set.

### GetKeywords

`func (o *EditCollectionsV2Request) GetKeywords() []string`

GetKeywords returns the Keywords field if non-nil, zero value otherwise.

### GetKeywordsOk

`func (o *EditCollectionsV2Request) GetKeywordsOk() (*[]string, bool)`

GetKeywordsOk returns a tuple with the Keywords field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetKeywords

`func (o *EditCollectionsV2Request) SetKeywords(v []string)`

SetKeywords sets Keywords field to given value.

### HasKeywords

`func (o *EditCollectionsV2Request) HasKeywords() bool`

HasKeywords returns a boolean if a field has been set.

### GetDatasets

`func (o *EditCollectionsV2Request) GetDatasets() []CreateCollectionsIntegrationsRequestDatasetsInner`

GetDatasets returns the Datasets field if non-nil, zero value otherwise.

### GetDatasetsOk

`func (o *EditCollectionsV2Request) GetDatasetsOk() (*[]CreateCollectionsIntegrationsRequestDatasetsInner, bool)`

GetDatasetsOk returns a tuple with the Datasets field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetDatasets

`func (o *EditCollectionsV2Request) SetDatasets(v []CreateCollectionsIntegrationsRequestDatasetsInner)`

SetDatasets sets Datasets field to given value.

### HasDatasets

`func (o *EditCollectionsV2Request) HasDatasets() bool`

HasDatasets returns a boolean if a field has been set.

### GetDur

`func (o *EditCollectionsV2Request) GetDur() []CreateCollectionsIntegrationsRequestDatasetsInner`

GetDur returns the Dur field if non-nil, zero value otherwise.

### GetDurOk

`func (o *EditCollectionsV2Request) GetDurOk() (*[]CreateCollectionsIntegrationsRequestDatasetsInner, bool)`

GetDurOk returns a tuple with the Dur field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetDur

`func (o *EditCollectionsV2Request) SetDur(v []CreateCollectionsIntegrationsRequestDatasetsInner)`

SetDur sets Dur field to given value.

### HasDur

`func (o *EditCollectionsV2Request) HasDur() bool`

HasDur returns a boolean if a field has been set.

### GetPublications

`func (o *EditCollectionsV2Request) GetPublications() []CreateCollectionsIntegrationsRequestDatasetsInner`

GetPublications returns the Publications field if non-nil, zero value otherwise.

### GetPublicationsOk

`func (o *EditCollectionsV2Request) GetPublicationsOk() (*[]CreateCollectionsIntegrationsRequestDatasetsInner, bool)`

GetPublicationsOk returns a tuple with the Publications field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetPublications

`func (o *EditCollectionsV2Request) SetPublications(v []CreateCollectionsIntegrationsRequestDatasetsInner)`

SetPublications sets Publications field to given value.

### HasPublications

`func (o *EditCollectionsV2Request) HasPublications() bool`

HasPublications returns a boolean if a field has been set.

### GetCollaborators

`func (o *EditCollectionsV2Request) GetCollaborators() []int32`

GetCollaborators returns the Collaborators field if non-nil, zero value otherwise.

### GetCollaboratorsOk

`func (o *EditCollectionsV2Request) GetCollaboratorsOk() (*[]int32, bool)`

GetCollaboratorsOk returns a tuple with the Collaborators field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetCollaborators

`func (o *EditCollectionsV2Request) SetCollaborators(v []int32)`

SetCollaborators sets Collaborators field to given value.

### HasCollaborators

`func (o *EditCollectionsV2Request) HasCollaborators() bool`

HasCollaborators returns a boolean if a field has been set.

### GetPublic

`func (o *EditCollectionsV2Request) GetPublic() bool`

GetPublic returns the Public field if non-nil, zero value otherwise.

### GetPublicOk

`func (o *EditCollectionsV2Request) GetPublicOk() (*bool, bool)`

GetPublicOk returns a tuple with the Public field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetPublic

`func (o *EditCollectionsV2Request) SetPublic(v bool)`

SetPublic sets Public field to given value.

### HasPublic

`func (o *EditCollectionsV2Request) HasPublic() bool`

HasPublic returns a boolean if a field has been set.

### GetStatus

`func (o *EditCollectionsV2Request) GetStatus() string`

GetStatus returns the Status field if non-nil, zero value otherwise.

### GetStatusOk

`func (o *EditCollectionsV2Request) GetStatusOk() (*string, bool)`

GetStatusOk returns a tuple with the Status field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetStatus

`func (o *EditCollectionsV2Request) SetStatus(v string)`

SetStatus sets Status field to given value.

### HasStatus

`func (o *EditCollectionsV2Request) HasStatus() bool`

HasStatus returns a boolean if a field has been set.


[[Back to Model list]](../README.md#documentation-for-models) [[Back to API list]](../README.md#documentation-for-api-endpoints) [[Back to README]](../README.md)


