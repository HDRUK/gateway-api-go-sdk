...
> CreateProjectGrant201Response CreateProjectGrant(ctx).Execute()
...
func main() {
	...
	resp, r, err := apiClient.ProjectGrantAPI.CreateProjectGrant(context.Background()).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `ProjectGrantAPI.CreateProjectGrant``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `CreateProjectGrant`: CreateProjectGrant201Response
	fmt.Fprintf(os.Stdout, "Response from `ProjectGrantAPI.CreateProjectGrant`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiCreateProjectGrantRequest struct via the builder pattern
